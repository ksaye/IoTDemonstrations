...
class IotHubWriterModule {
  ...
  on_message(message) {
      //console.log(`iothub.on_message`);
      try {
          let properties = {};
          for (var property in message.properties.propertyList) {
              properties[message.properties.propertyList[property].key] = message.properties.propertyList[property].value;
          }
          ...
          // we have to mark the message complete, else IoT Hub will not know it was processed.
          this.iothub_client.complete(message);
      }
      catch (err) {
          this.iothub_client.abandon(message);
          console.error(err);
      }
  }
